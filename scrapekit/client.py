...
from .exceptions import ConfigError, ScrapekitError
...
class Scraper:
    ...
    Args:
        config: A prebuilt :class:`~scrapekit.config.ScraperConfig`. Mutually
    ...
    async def fetch(self, url: str, **options: Any) -> FetchResponse:
        ...
        Args:
            url: Absolute URL.
            **options: Any :class:`~scrapekit.config.FetchOptions` field —
                ``method``, ``params``, ``data``, ``json``, ``headers``,
                ``cookies``, ``proxy``, ``timeout``, ``strategies``, and the
                browser waiting knobs.

        Returns:
            A :class:`~scrapekit.models.FetchResponse`. ``strategy_used`` records
        ...
        **options: Any,
        ...
        Args:
            ...
            parser: ``"llm"`` (default), ``"css"``, ``"jsonpath"``, or a custom
                :class:`~scrapekit.parsers.base.BaseParser`.
    ...
    async def extract(
        ...
        *,
        ...
        Args:
            content: Raw text, or a :class:`~scrapekit.models.FetchResponse`
