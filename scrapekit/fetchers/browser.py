...
from ..fingerprint import UserAgentProfile, build_headers, profile_for_user_agent
...
class PlaywrightFetcher(BaseFetcher):
    ...
    def _context_kwargs(self, options: FetchOptions) -> tuple[dict, object]:
        ...
        # Playwright sets User-Agent and Accept-Language from the context; passing
        # them again as extra headers can produce duplicates on some versions.
        extra_headers = {
            k: v for k, v in headers.items() if k.lower() not in ("user-agent", "accept-language")
        }

        kwargs: dict[str, object] = {
            "user_agent": profile.user_agent,
            "locale": profile.accept_language.split(",", 1)[0],
