...
```bash
pip install scrapeforge                # core: httpx + curl_cffi + pydantic + selectolax
pip install "scrapeforge[browser]"     # + playwright
pip install "scrapeforge[llm]"         # + instructor / anthropic / openai
pip install "scrapeforge[tls]"         # + tls-client
...
`scrapeforge/utils/detect.py` and matches interstitial markers, WAF status codes paired with tiny or
...
data-protection rules (including GDPR/CCPA where they apply).** Do not use scrapeforge to access data
...
- **No CAPTCHA solving, no challenge bypass.** Challenges are detected and surfaced as
  ...
  scrapeforge will not do it for you.
...
| Field | Value |
|---|---|
| PyPI project name | `scrapeforge` |
| Owner | `prashant-cr` |
| Repository | `scrapeforge` |
...
pip install -i https://test.pypi.org/simple/ \
            --extra-index-url https://pypi.org/simple/ scrapeforge
